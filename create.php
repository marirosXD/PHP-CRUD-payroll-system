...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO employees (name, position, rate_per_day, days_absent, total_work_days, gross_pay) VALUES (?, ?, ?, ?, ?, ?)");
    
    if ($stmt->execute([$name, $position, $rate_per_day, $days_absent, $total_work_days, $gross_pay])) {
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <select id="position" name="position" required>
                            ...
                            <option value="Manager">Manager - ₱1,500/day</option>
                            <option value="Supervisor">Supervisor - ₱1,200/day</option>
                            <option value="Staff">Staff - ₱900/day</option>
                            <option value="Intern">Intern - ₱500/day</option>
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label for="rate_per_day">Rate per Day (₱):</label>
                        <input type="number" id="rate_per_day" name="rate_per_day" step="0.01" required placeholder="0.00">
                    </div>
                    
                    <div class="form-group">
                        <label for="days_absent">Days Absent:</label>
                        <input type="number" id="days_absent" name="days_absent" required min="0" value="0">
                    </div>
                    
                    <div class="form-group">
                        <label for="total_work_days">Total Work Days:</label>
                        <input type="number" id="total_work_days" name="total_work_days" required min="0" value="22">
                    </div>
                </div>
